...
namespace BirdmanStudio
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            logFile.Close();
            //var noo = Helper.MagicToSignature("MODF");
            ...
            //M2Converter m2convert = new M2Converter(fileName, true);
            //AdtConverter adtconvert = new AdtConverter(fileName, true, true);
            ...
            if (File.Exists(fileName))
            {
                ...
                using (BinaryReader reader = new BinaryReader(File.Open(fileName, FileMode.Open)))
                {
                    ...
                    //var isDBC = Helper.SeekChunkM2(reader, "WDBC");
                    /*
                    if (isDBC)
                    {
                        try
                        {
                            DBC.DBC_Header dbc = new DBC.DBC_Header();
                            dbc.record_count = reader.ReadUInt32();
                            Console.WriteLine(dbc.record_count);
                    ...
                    */
                    ...
                    if (isAdt == true)
                    {
                        try
                        {
                            ...
                            for (uint i = 0; i < ADTmddf.MagicSize; i++)
                            {
                                ...
                                {
                                    file.WriteLine("DoodadDefs[" + i.ToString() + "]" + " " + ADTmddf.FileDataID);
